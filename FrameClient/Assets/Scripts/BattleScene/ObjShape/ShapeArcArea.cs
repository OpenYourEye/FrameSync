...
public class ShapeArcArea : ShapeBase {
	...
	public override bool IsCollisionCircle(GameVector2 _pos,int _radius){
		return ToolGameVector.CollideCircleAndArcArea (_pos,_radius,arcCenter,arcRadius,arcAngle,arcAngleSize);
	}

	public override bool IsCollisionCircleCorrection(GameVector2 _pos,int _radius,out GameVector2 _amend){
		return ToolGameVector.CollideCircleAndArcArea (_pos,_radius,arcCenter,arcRadius,arcAngle,arcAngleSize,out _amend);
	}
	...
	void OnDrawGizmos()
	{
		...
		for (int i = 1; i <= vectNumber; i++) {
			...
			if (i == vectNumber) {
				Gizmos.DrawLine (arcCenter,firstVect1);
				Gizmos.DrawLine (arcCenter,firstVect2);
			}
		}
